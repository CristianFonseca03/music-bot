...
module.exports = class Loop extends Interaction {
  ...
  async exec(int, data) {
    ...
    if (members.size > 1 && !isDJ && !int.member.permissions.has('MANAGE_GUILD')) {
      return int.reply({
        content: 'You must be a DJ or be alone in the voice channel to use this command!',
        ephemeral: true,
      });
    }
    ...
    if (mode === 'track') {
      if (queue.repeatMode === RepeatMode.SONG) {
        ...
        return int.reply({
          content: `${this.client.emotes.get('looped')} Looped the current track!`,
          ephemeral: true,
        });
      }
    } else if (mode === 'queue') {
      if (queue.repeatMode === RepeatMode.QUEUE) {
        ...
        return int.reply({
          content: `${this.client.emotes.get('looped')} Looped the current queue!`,
          ephemeral: true,
        });
      }
      ...
      if (queue.repeatMode === RepeatMode.DISABLED) {
        ...
        return int.reply({
          content: `${this.client.emotes.get('noloop')} Disabled the loop mode!`,
          ephemeral: true,
        });
      }
    }
