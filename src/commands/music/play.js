...
module.exports = class Play extends Interaction {
  ...
  async exec(int, data) {
    ...
    if (force && members.size > 1 && !isDJ && !int.member.permissions.has('MANAGE_GUILD')) {
      return int.reply({
        content: 'You must be a DJ or be alone in the voice channel to use the force function!',
        ephemeral: true,
      });
    }
    ...
    if (isPlaylist) {
      return this.client.play(this.client, int, data, song, source, true, false, false, false);
    } else {
      return this.client.play(this.client, int, data, song, source, false, false, false, force);
    }
  }
};
