...
module.exports = class Search extends Interaction {
  ...
  async exec(int, data) {
    ...
    int.reply({
      content: `${this.client.emotes.get('search')} Searching \`${input}\` ${this.client.emotes.get(
        'youtube',
      )}`,
    });
    ...
    let results = await DMP.Utils.search(input, { requestedBy: int.user }, queue, 10).catch((e) => {
      console.log(e);
    });
    ...
    let selection = await int.channel
      ...
      .then((m) => {
        ...
        return this.client.play(this.client, int, data, song, 'youtube', false, true);
      })
      .catch(() => {
        return int.editReply({
